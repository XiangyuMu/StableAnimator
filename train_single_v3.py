...
from animation.dataset.animation_dataset_v1 import LargeScaleAnimationVideos
...
from animation.modules.head_model import FusionFaceId
from torch.utils.data import DataLoader

from animation.pipelines.validation_pipeline_animation_v1 import ValidationAnimationPipeline
import transformers
...
def validate_and_convert_image(image, target_size=(256, 512)):
    if image is None:
...
def create_image_grid(images, rows, cols, target_size=(256, 512)):
...
def parse_args():
    ...
    parser.add_argument(
        "--validation_face_pose",
        type=str,
        default=None,
        help=(
            "the validation face pose"
        ),
    )
...
def save_frames_to_video(frames, output_path, fps=30):
    ...
    # 将每一帧写入视频
    for frame in frames:
        # 将PIL Image转换为numpy数组，并从RGB转换为BGR（OpenCV使用BGR）
...
def find_mask_coordinates(tensor):
    ...
    for frame in range(frames):
        ...
        coords = torch.nonzero(mask == 1.0, as_tuple=False)  # 找到 mask 区域的坐标
        if coords.numel() > 0:
...
def find_and_expand_masks(tensor):
    ...
    # 计算需要添加的像素数，使宽和高都能被8整除
    width_padding = (8 - width % 8) % 8
    height_padding = (8 - height % 8) % 8
    
    # 均匀分布padding到两侧
    min_x = max(0, min_x - width_padding // 2)
    max_x = min(w - 1, max_x + (width_padding - width_padding // 2))
    min_y = max(0, min_y - height_padding // 2)
    max_y = min(h - 1, max_y + (height_padding - height_padding // 2))
    
    # 再次检查并调整，确保最终的宽高能被8整除
    width = max_x - min_x + 1
    height = max_y - min_y + 1
    ...
    if width % 8 != 0:
        ...
        if min_x >= padding:
            min_x -= padding
        elif max_x + padding < w:
            max_x += padding
        else:
            # 如果两边都没有足够空间，则从可用空间中分配
            left_space = min_x
            right_space = w - 1 - max_x
            left_padding = min(left_space, padding // 2)
            right_padding = min(right_space, padding - left_padding)
            min_x -= left_padding
            max_x += right_padding
    ...
    if height % 8 != 0:
        ...
        if min_y >= padding:
            min_y -= padding
        elif max_y + padding < h:
            max_y += padding
        else:
            # 如果两边都没有足够空间，则从可用空间中分配
            top_space = min_y
            bottom_space = h - 1 - max_y
            top_padding = min(top_space, padding // 2)
            bottom_padding = min(bottom_space, padding - top_padding)
            min_y -= top_padding
            max_y += bottom_padding
    
    # 扩充所有 mask 为矩形
    for frame in range(frames):
        mask = tensor[0, frame, 0]
        mask[:, :] = 0  # 清空原始 mask
        mask[min_y:max_y+1, min_x:max_x+1] = 1  # 填充矩形区域
    
    # 最终检查
    final_width = max_x - min_x + 1
    final_height = max_y - min_y + 1
    assert final_width % 8 == 0, f"Width {final_width} is not divisible by 8"
    assert final_height % 8 == 0, f"Height {final_height} is not divisible by 8"
...
def main():
    ...
    accelerator = Accelerator(
        ...
        # distributed_type="GLOO",
        # kwargs_handlers=[ddp_kwargs] 
    )      # 设置accelerate， log、mixed_precision、梯度累计
    ...
    if args.report_to == "wandb":
        ...
        wandb.login(key='8a5ca5be6185ae9daec48fad766bbf1c0b876a01')
        
        # Initialize wandb
        run_name = f"StableAnimator-run-{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}"
        wandb.init(
            project="StableAnimator",
            name=run_name,
            config=vars(args),
            resume="allow"
        )
    ...
    pose_net = PoseNet(noise_latent_channels=unet.config.block_out_channels[0])  # (16, 320, 64, 64)
    headpose_net = HeadPoseNet(noise_latent_channels=unet.config.block_out_channels[0])  # (16,320,64,64)
    ...
    if args.finetune_mode is True and args.posenet_model_finetune_path is not None and args.headpose_finetune_path is not None and args.face_encoder_finetune_path is not None and args.unet_model_finetune_path is not None:
        ...
        if args.posenet_model_finetune_path.endswith(".pth"):
            ...
            print(1 / 0)
        if args.headpose_finetune_path.endswith(".pth"):
            headpose_net_state_dict = torch.load(args.headpose_finetune_path, map_location="cpu")
            headpose_net.load_state_dict(headpose_net_state_dict, strict=True)
        else:
            print("headpose_net weights loading fail")
            print(1 / 0)
    ...
    headpose_net.requires_grad_(False)
    ...
    headpose_net.requires_grad_(True)
    ...
    for name, para in headpose_net.named_parameters():
        para.requires_grad = True
        parameters_list.append({"params": para, "lr": args.learning_rate})
    ...
    unet, pose_net, headpose_net,face_encoder,  optimizer, lr_scheduler, train_dataloader = accelerator.prepare(
        unet, pose_net,headpose_net,face_encoder, optimizer, lr_scheduler, train_dataloader
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(pose_net, headpose_net, unet, face_encoder):
                with accelerator.autocast():
                    pixel_values = batch["pixel_values"].to(weight_dtype).to(
                        accelerator.device, non_blocking=True
                    )
                    # print('pixel_values', pixel_values.shape)
                    conditional_pixel_values = batch["clothes_pixels"].to(weight_dtype).to(
                        accelerator.device, non_blocking=True
                    )
                    ...
                    # print("This is the pixel_values: ", pixel_values.size())
                    # print("This is the conditional_pixel_values: ", conditional_pixel_values.size())
                    latents = tensor_to_vae_latent(pixel_values, vae).to(dtype=weight_dtype)

                    # Get the text embedding for conditioning.
                    encoder_hidden_states = encode_image(cloth_pixel_values).to(dtype=weight_dtype)
                    ...
                    pose_head_pixels = batch["pose_with_head_image"].to(
                        dtype=weight_dtype, device=accelerator.device, non_blocking=True
                    )
                    head_pixels = batch["reference_head_image"].to(
                        dtype=weight_dtype, device=accelerator.device, non_blocking=True
                    )
                    ...
                    # print('pose_latents', pose_latents.size())
                    pose_head_latents = headpose_net(pose_head_pixels)
                    # print('pose_head_latents', pose_head_latents.size())
                    head_latents = tensor_to_vae_latent(head_pixels, vae, scale=False) # [1,4,64,64]
                    # print('head_latents', head_latents.size())

                    # print('faceid_embeds', faceid_embeds.size())
                    # print('encoder_hidden_states', encoder_hidden_states.size())

                    faceid_latents = face_encoder(pose_head_latents, head_latents, faceid_embeds, pose_latents)
                    # print('faceid_latents', faceid_latents.size())



                    # head_latents = tensor_to_vae_latent(head_pixels, vae).to(dtype=weight_dtype)
                    ...
                    num_frames = faceid_latents.size(0)
                    ...
                    # print("This is the size of encoder_hidden_states: ", encoder_hidden_states.size())
                    # Predict the noise residual
                    # print('timesteps.value', timesteps)
                    # print('inp_noisy_latents', inp_noisy_latents.size())
                    ...
                    # print('tgt_face_masks', tgt_face_masks.size())
                    ...
                    head_masks_orignal = batch["head_mask_rectangle"].to(
                        dtype=weight_dtype, device=accelerator.device, non_blocking=True
                    )

                    head_masks_orignal,(min_x, min_y, max_x, max_y) = find_and_expand_masks(head_masks_orignal)
                    target_head_image = pixel_values[:,:,:,min_y:max_y+1,min_x:max_x+1]
                    # print('target_head_image', target_head_image.shape)
                   
                   
                    head_masks_latents = rearrange(head_masks_orignal, "b f c h w -> (b f) c h w")
                    head_masks_latents = F.interpolate(head_masks_latents, size=(target.size()[-2], target.size()[-1]),
                                                   mode='nearest')
                    head_masks_latents = rearrange(head_masks_latents, "(b f) c h w -> b f c h w", f=args.sample_n_frames)

                    (min_x, min_y, max_x, max_y) = find_mask_coordinates(head_masks_latents)
                    ...
                    frames_denosie = decode_latents(vae, denoised_latents[:,:,:,min_y:max_y+1,min_x:max_x+1], 8, 1)
                

                    frames_denosie_predict = rearrange(frames_denosie, "b c f h w -> b f c h w")

                    # print("frames_denosie_predict.shape ", frames_denosie_predict.shape)

                    # frames_1 = tensor2vid(frames_denosie, image_processor, output_type='pil')

                    # timestamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
                    # output_video_path = f"/data/muxiangyu/pythonPrograms/StableAnimator/denoised_frames_{timestamp}.mp4"
                    # save_frames_to_video(frames_1[0], output_video_path, fps=10)

                    # head_masks,(min_x, min_y, max_x, max_y) = find_and_expand_masks(head_masks_orignal)
                    # target_head_image = pixel_values[:,:,:,min_y:max_y,min_x:max_x]
                    # # Convert the target_head_image directly to a list of PIL images
                    # print("target_head_image shape:", target_head_image.shape)
                    # try:
                    #     # 首先确保target_head_image是正确的格式
                    #     # tensor2vid期望的输入格式是[batch_size, channels, num_frames, height, width]
                    #     # 确保通道数为3（RGB）
                    #     if target_head_image.shape[2] != 3:
                    #         print(f"Warning: Channel dimension is {target_head_image.shape[2]}, expected 3")
                        
                    #     # 使用手动方法将帧转换为PIL图像
                    #     target_frames_pil = []
                    #     batch_size, num_frames, channels, height, width = target_head_image.shape
                        
                    #     for f in range(num_frames):
                    #         # 获取单个帧并转换为适合PIL的格式
                    #         frame = target_head_image[0, f].permute(1, 2, 0)  # [H, W, C]
                            
                    #         # 规范化值到0-255范围
                    #         if frame.min() < 0 or frame.max() > 1:
                    #             frame = (frame + 1.0) / 2.0 * 255  # 从[-1,1]转换到[0,255]
                    #         frame = frame.clamp(0, 255).to(torch.uint8).cpu().numpy()
                            
                    #         # 转换为PIL图像
                    #         pil_frame = Image.fromarray(frame)
                    #         target_frames_pil.append(pil_frame)
                        
                    #     # 现在我们有了PIL图像列表，可以继续保存
                    #     timestamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
                    #     output_video_path = f"/data/muxiangyu/pythonPrograms/StableAnimator/target_head_image_{timestamp}.mp4"
                    #     save_frames_to_video(target_frames_pil, output_video_path, fps=10)
                        
                    #     # 创建GIF
                    #     output_gif_path = output_video_path.replace('.mp4', '.gif')
                    #     create_gif(target_frames_pil, output_gif_path, fps=10)
                    ...
                    #     # 保存tensor数据为numpy数组，以便后续分析
                    #     np_path = f"/data/muxiangyu/pythonPrograms/StableAnimator/target_head_tensor_{timestamp}.npy"
                    #     np.save(np_path, target_head_image.detach().cpu().numpy())
                    #     print(f"Saved tensor data to {np_path} for further analysis")


                    target_head_image = target_head_image.to(accelerator.device)
                    frames_denosie_predict = frames_denosie_predict.to(accelerator.device)

                    batch_de, frame_de, channels_de, height_de, width_de = frames_denosie_predict.shape

                    # 随机选取一个帧索引
                    # random_frame_idx = random.randint(0, frame_de)
                    # 选择随机帧
                    # frames_denosie_predict = frames_denosie_predict[0, random_frame_idx, :, :, :]
                    # 将其转换为numpy数组
                    ...
                    l2_loss = l2_loss_criterion(target_head_image, frames_denosie_predict)

                    vgg_loss = VGGLossForVideos(accelerator.device)
                    # print('target_head_image', target_head_image.shape)
                    # print('frames_denosie_predict', frames_denosie_predict.shape)
                    vgg_loss_value = vgg_loss(target_head_image, frames_denosie_predict)

                    flow_loss_fn = OpticalFlowLoss(loss_type='smooth_l1')
                    flow_loss = flow_loss_fn(frames_denosie_predict, target_head_image)
                    
                    

                    loss = loss_ldm + l2_loss * 0.3 + 0.01 * vgg_loss_value + 0.01 * flow_loss

                    # print('loss_ldm', loss_ldm)
                    ...
                    #     dim=1,
                    # )
                    # print("This is the loss 0: ", loss)
                    # loss = loss.mean()
                    # print("This is the loss 1: ", loss)
                    # Gather the losses across all processes for logging (if we use distributed training).

                    # print(f"Loss shape: {loss.shape}")
                    # print(f"Repeated loss shape: {loss.repeat(args.per_gpu_batch_size).shape}")
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0 and accelerator.is_main_process:
                    ...
                    unwrap_headpose_net = accelerator.unwrap_model(headpose_net)
                    ...
                    unwrap_headpose_net_state_dict = unwrap_headpose_net.state_dict()
                    torch.save(unwrap_headpose_net_state_dict, os.path.join(args.output_dir, f"checkpoint-{global_step}",
                                                                            f"headpose_net-{global_step}.pth"))
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.validation_steps == 0:
                        ...
                        log_validation(
                            ...
                            validation_face_pose=args.validation_face_pose,
...
def log_validation(
    ...
    validation_headpose_net = accelerator.unwrap_model(headpose_net)
    ...
    pipeline = ValidationAnimationPipeline(
        ...
        headpose_net=validation_headpose_net,
    ...
    validation_face_pose = Image.open(validation_face_pose).convert('RGB')
