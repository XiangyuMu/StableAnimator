        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(pose_net, headpose_net, unet, face_encoder):
                with accelerator.autocast():
                    ...
                    frames_denosie = decode_latents(vae, denoised_latents[:,:,:,:53,:], 8, 1)
                    print(type(frames_denosie))

                    print("frames_denosie.shape ", frames_denosie.shape)
                    frames_1 = tensor2vid(frames_denosie, image_processor, output_type='pil')
                    print('frame_length',len(frames_1))
                    print('frame_length1',len(frames_1[0]))

                    print(type(frames_1[0][0]))
                    

                    print("denoise_latents.shape ",denoised_latents.shape)
                    timestamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
