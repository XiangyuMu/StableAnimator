...
class ValidationAnimationPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        # image_cloth = image_cloth.resize((self.width, self.height))
        # image_cloth = torch.from_numpy(np.array(image_cloth)).float()
        # image_cloth = image_cloth / 127.5 - 1      # normalize to [-1, 1]   clothes image

        # clothes_vae_latents = self._encode_vae_image(image_cloth, device, num_videos_per_prompt, do_classifier_free_guidance)
        ...
        with self.progress_bar(total=len(timesteps) * len(indices)) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                for idx in indices:
                    ...
                    pose_head_latent = self.headpose_net(image_head_pose.to(device))
