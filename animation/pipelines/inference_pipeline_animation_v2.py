...
class InferenceAnimationPipeline(DiffusionPipeline):
    ...
    def _encoder_vea_tensor(self,
        ...
        if len(image.shape) == 5:
            ...
            latents = self.vae.encode(image).latent_dist.mode()
        if do_classifier_free_guidance:
            negative_image_latents = torch.zeros_like(latents)
            latents = torch.cat([negative_image_latents, latents])
    ...
    def __call__(
        ...
        image_embeddings = self._encode_image(image, device, num_videos_per_prompt, do_classifier_free_guidance)  # head
        ...
        for clothes in image_clothes:
            clothes = torch.from_numpy(np.array(clothes)).float().to(device)
        ...
        image_head_pose = image_head_pose.resize((width, height))
        image_head_pose = torch.from_numpy(np.array(image_head_pose)).float()
        image_head_pose = image_head_pose / 127.5 - 1      # normalize to [-1, 1]   head image
        image_head_pose = rearrange(image_head_pose, "h w c -> c h w")
        ...
        with self.progress_bar(total=len(timesteps) * len(indices)) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                # print('image_latents size',image_latents.size())
                ...
                for idx in indices:
                    ...
                    pose_latents = self.pose_net(pose_pil_image_list[idx].to(device=device, dtype=image_embeddings.dtype))
                    pose_head_latent = self.headpose_net(image_head_pose.to(device=device, dtype=image_embeddings.dtype))
                    # pose_latents = pose_latents.unsqueeze(0)
                    pose_head_latent = pose_head_latent.unsqueeze(0)

                    # print('pose_latents size',pose_latents.size())
                    # print('pose_head_latent size',pose_head_latent.size())
                    # print('face_latent size',face_latent.size())
                    # print('validation_image_id_ante_embedding size',validation_image_id_ante_embedding.size())
                    # print('image_embeddings size',image_embeddings.size())

                    print('pose_head_latent dtype',pose_head_latent.dtype)
                    print('face_latent dtype',face_latent.dtype)
                    print('validation_image_id_ante_embedding dtype',validation_image_id_ante_embedding.dtype)
                    print('image_embeddings dtype',image_embeddings.dtype)
                    print('pose_latents dtype',pose_latents.dtype)

                    pose_head = self.face_encoder(pose_head_latent, face_latent[:1], validation_image_id_ante_embedding, pose_latents)
                    ...
                    cond_image_embeddings = torch.cat([cond_image_embeddings, pose_head], dim=1)
                    image_embeddings_final = torch.cat([uncond_image_embeddings, cond_image_embeddings])


                    # print('latent_model_input size',latent_model_input.size())
                    # print('t size',t)
                    # print('image_embeddings size',image_embeddings.size())
                    # print('added_time_ids size',added_time_ids.size())
                    # print('pose_latents size',pose_latents.size())
