...
def load_images_from_folder(folder, width, height):
    ...
    return images
def load_original_images(folder, width, height):
    images = []
    files = os.listdir(folder)
    png_files = [f for f in files if f.endswith('.png')]
    png_files.sort(key=lambda x: int(x.split('_')[1].split('.')[0]))  # 确保顺序一致
    
    for filename in png_files:
        img = Image.open(os.path.join(folder, filename)).convert('RGB')  # 读取 RGB
        img = img.resize((width, height))  # 调整大小
        # img = np.array(img, dtype=np.float32) / 255.0  # 归一化
        images.append(img)

    # 转换为 (num_frames, H, W, 3) 并转换为 PyTorch Tensor
...
def load_masks(folder, width, height):
    """
    从文件夹中加载mask图像
    
    Args:
        folder (str): mask图像所在文件夹路径
        width (int): 目标宽度
        height (int): 目标高度
        
    Returns:
        torch.Tensor: shape为(num_frames, 1, H, W)的mask tensor，值为0或1
    """
    masks = []
    files = os.listdir(folder)
    png_files = [f for f in files if f.endswith('.png')]
    ...
    for filename in png_files:
        # 读取mask图像（转为灰度图）
        mask = Image.open(os.path.join(folder, filename)).convert('L')
        # 调整大小
        mask = mask.resize((width, height))
        # 转换为numpy数组并二值化（大于127的为1，小于等于127的为0）
...
if __name__ == "__main__":
    ...
    original_images = load_original_images(args.original_images_folder, width=args.width, height=args.height)
    masks = load_masks(args.masks_folder, width=args.width, height=args.height)
