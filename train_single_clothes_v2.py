...
def parse_args():
    ...
    parser.add_argument(
        "--cloth_encoder_model_finetune_path",
        type=str,
        default=None,
        help="Path to the pretrained cloth encoder model",
    )
    parser.add_argument(
...
def main():
    ...
    if args.finetune_mode is True and args.posenet_model_finetune_path is not None and args.face_encoder_finetune_path is not None and args.unet_model_finetune_path is not None:
        ...
        if args.posenet_model_finetune_path.endswith(".pth"):
            ...
            print(1 / 0)
        if args.cloth_encoder_model_finetune_path.endswith(".pth"):
            cloth_encoder_state_dict = torch.load(args.cloth_encoder_model_finetune_path, map_location="cpu")
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(pose_net, cloth_encoder, face_encoder, unet):
                with accelerator.autocast():
                    ...
                    reference_cloth_image = batch["reference_cloth_image"].to(weight_dtype).to(
                        accelerator.device, non_blocking=True
                    )
                    ...
                    encoder_hidden_states = encode_image(reference_cloth_image).to(dtype=weight_dtype)
                    ...
                    clothes_latents = cloth_encoder(clothes_pixels)
