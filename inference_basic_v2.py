...
from animation.pipelines.inference_pipeline_animation_v2 import InferenceAnimationPipeline
...
if __name__ == "__main__":
    ...
    headpose_net = HeadPoseNet(noise_latent_channels=unet.config.block_out_channels[0])
    ...
    if args.posenet_model_name_or_path is not None and args.headpose_name_or_path is not None and args.face_encoder_model_name_or_path is not None and args.unet_model_name_or_path is not None:
        ...
        if args.posenet_model_name_or_path.endswith(".pth"):
            ...
            print(1/0)
        if args.headpose_name_or_path.endswith(".pth"):
            headpose_net_state_dict = torch.load(args.headpose_name_or_path, map_location="cpu")
    ...
    validation_image = Image.open(args.validation_image).convert('RGB')  # head
    validation_control_images = load_images_from_folder(args.validation_control_folder, width=args.width, height=args.height)   #pose
    validation_clothes_images = load_images_from_folder(args.validation_clothes_folder, width=args.width, height=args.height)   # clothes
    ...
    validation_face_pose = Image.open(args.validation_headpose).convert('RGB')

    print("Start to generate video")
    print("The number of frames is: ", num_frames)
    print('validation_image', validation_image.size)  # (256, 512)
    print('validation_control_images', np.array(validation_control_images[0]).shape)  # (512, 256, 3)
    print('validation_clothes_images', np.array(validation_clothes_images[0]).shape)  # (512, 256, 3)
    print('validation_face_pose', validation_face_pose.size)  # (256, 512)


    decode_chunk_size = args.decode_chunk_size
    video_frames = pipeline(
        image=validation_image,
        image_cloth=validation_clothes_images[0],
